...
(define M_state
  (lambda (statement state)
    (printf "M_state called with statement: ~s and state: ~s\n" statement state)
    (cond
...
(define M_if
  (lambda (statement state)
    (printf "M_if called with statement: ~s, state: ~s\n" statement state)
...
(define M_declare
  (lambda (statement state)
    (printf "M_declare called with statement: ~s and state: ~s\n" statement state)
    (cond
...
(define M_assign
  (lambda (statement state)
    (printf "M_assign called with statement: ~s and state: ~s\n" statement state)
...
(define M_value
  (lambda (expression state)
    (printf "M_value called with expression: ~s and state: ~s\n" expression state)
    (cond
      ; logical evaluation
...
(define subtract
  (lambda (expression state)
    (printf "subtract called with expression: ~s and state: ~s\n" expression state)
    (cond 
...
(define M_boolean
  (lambda (expression state)
    (printf "M_boolean called with expression: ~s and state: ~s\n" expression state)
    (cond
      ((boolean? expression) expression)
      ((var? expression) (get_var expression state))
      ((eq? 'true expression) #t)
...
(define M_return
  (lambda (statement state)
    (printf "M_return called with statement: ~s and state: ~s\n" statement state)
...
(define var_dec
  (lambda (var state)
    (printf "var_dec called with var: ~s and state: ~s\n" var state)
    (append_state var 'null state)))

; Variable declaration with assignment, assigns var to val
(define var_dec_assn
  (lambda (var val state)
    (printf "var_dec_assn called with var: ~s, val: ~s and state: ~s\n" var val state)
    (append_state var val state)))

(define var_assn
  (lambda (var val state)
    (printf "var_assn called with var: ~s, val: ~s and state: ~s\n" var val state)
...
(define append_state 
  (lambda (var val old_state)
    (printf "append_state called with var: ~s, val: ~s and old_state: ~s\n" var val old_state)
...
(define append_var
  (lambda (var var_list)
    (printf "append_var called with var: ~s and var_list: ~s\n" var var_list)
    (cons var var_list)))

; Appends a variable to the variable list within state
(define append_val
  (lambda (val val_list)
    (printf "append_val called with val: ~s and val_list: ~s\n" val val_list)
    (cons val val_list)))
...
(define var_exists? 
  (lambda (var state)
    (printf "var_exists? called with var: ~s and state: ~s\n" var state)
    (cond 
      ((null? (car state)) #f)
...
(define var_init?
  (lambda (var state)
    (printf "var_init? called with var: ~s and state: ~s\n" var state)
    (cond
...
(define remove_binding
  (lambda (var state)
    (printf "remove_binding called with var: ~s and state: ~s\n" var state)
...
(define find_replace_val
  (lambda (var var_list val_list)
    (printf "find_replace_val called with var: ~s, var_list: ~s, val_list: ~s\n" var var_list val_list)
    (cond
...
(define add_binding
  (lambda (var val state)
    (printf "add_binding called with var: ~s, val: ~s, state: ~s\n" var val state)
...
(define find_set_val
  (lambda (var val var_list val_list)
    (printf "find_set_val called with var: ~s, val: ~s, var_list: ~s, val_list: ~s\n" var val var_list val_list)
    (cond
      ((null? var_list) val_list) ;
...
(define get_var
  (lambda (var state)
    (printf "get_var called with var: ~s and state: ~s\n" var state)
    (cond
...
(define find_var
  (lambda (var state)
    (printf "find_var called with var: ~s and state: ~s\n" var state)
    (cond
...
(define find_var_helper
  (lambda (value)
    (printf "find_var_helper called with value: ~s\n" value)
    (cond
